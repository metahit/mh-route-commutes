...
# Define LA and mode
  region_lad14 <- read.csv("0_DataInput/lad14cd.csv")
  region_lad14 <- region_lad14[region_lad14$run==1,]
  commutemode <- 1
  maxdist_mode <- 15
...
# Load and subset census lines to be routed
  ...
  lines <- lines[lines$home_lad14cd %in% region_lad14$home_lad14cd,]
...
# Create and populate matrix for between-routes
  ...
  for(j in 1:length(region_lad14$home_lad14cd)){
  for(k in 1:length(region_lad14$home_lad14cd)){
  lahome <- as.character(region_lad14$home_lad14cd[j])
  latravel <- as.character(region_lad14$home_lad14cd[k])
  ...
  if(is.null(intersect(routes[routes@data$home_lad14cd==lahome,], lad14[lad14@data$lad15cd==latravel,]))) {
    mat$length[(mat$lahome==lahome & mat$latravel==latravel)] <- 0
  } else {
...
# Add in a bit extra to capture within-LSOA routes
  ...
  for(j in 1:length(region_lad14$home_lad14cd)){
   lahome <- as.character(region_lad14$home_lad14cd[j])
   latravel <- lahome
   mat$length[(mat$lahome==lahome & mat$latravel==latravel)] <-  as.numeric(mat$length[(mat$lahome==lahome & mat$latravel==latravel)] 
                                                                + (lsoa_within_dist*as.numeric(sum(lines_within$home_lad14cd==lahome))))
  }
  
# Save matrix
  write_csv(mat, file.path(paste0("1_DataCreated/2_travelmatrix_mode", commutemode, ".csv")))
